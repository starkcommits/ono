import CricketImage from '@/assets/CricketImage.svg'
import DownArrowIcon from '@/assets/DownArrowIcon.svg'
import { Separator } from '@/components/ui/separator'
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs'
import X from '@/assets/X.svg'

import CircleCrossIcon from '@/assets/CircleCrossIcon.svg'
import { useEffect, useRef, useState } from 'react'

const ActiveMarketHoldings = () => {
  const [activeMarketHoldingPortfolioTab, setActiveMarketHoldingPortfolioTab] =
    useState(localStorage.getItem('activeMarketHoldingPortfolioTab') || 'all')

  const handleTabChange = (value) => {
    localStorage.setItem('activeMarketHoldingPortfolioTab', value)
    setActiveMarketHoldingPortfolioTab(value)
  }

  const tabKeys = ['all', 'matched', 'exiting', 'exited', 'cancelled']
  const tabRefs = tabKeys.reduce((acc, key) => {
    acc[key] = useRef(null)
    return acc
  }, {})

  useEffect(() => {
    if (tabRefs[activeMarketHoldingPortfolioTab]?.current) {
      tabRefs[activeMarketHoldingPortfolioTab].current.scrollIntoView({
        behavior: 'smooth',
        inline: 'center',
        block: 'nearest',
      })
    }
  }, [activeMarketHoldingPortfolioTab])

  return (
    <div className="leading-[100%] bg-[#F5F5F5] w-full min-h-screen overflow-y-auto">
      <div className="sticky z-[50] top-0 left-0 right-0 flex flex-col font-inter max-w-md mx-auto pt-4 bg-white mb-auto w-full">
        <div className="border-[0.33px] border-x-0 border-t-0 border-b border-[#DBC5F7] w-full flex justify-center pb-2">
          <h1 className="text-xl font-[500] text-[#2C2D32] mx-auto">
            My Portfolio
          </h1>
        </div>
      </div>
      <div className="flex flex-col w-full">
        <div className="flex flex-col gap-4 items-center pb-8 pt-4">
          <div>
            <img src={CricketImage} alt="" />
          </div>
          <div>
            <p className="font-normal text-sm text-[#2C2D32]">
              Hyderabad to win the match vs Mumbai?
            </p>
          </div>
          <div>
            <p className="text-xs font-normal text-[#5F5F5F]">
              THE MARKET PREDICTS
            </p>
          </div>
          <div>
            <p className="font-normal text-[22px] text-[#007AFF]">
              53% probability of YES
            </p>
          </div>
        </div>
        <div className="p-4">
          <div className="flex flex-col gap-4 items-center p-4 rounded-[5px] bg-white">
            <div className="flex flex-col gap-1 items-center">
              <p className="font-normal text-xs">Live Returns</p>
              <p className="flex items-center gap-1">
                <span>
                  <img src={DownArrowIcon} alt="" />
                </span>
                <span className="font-inter font-semibold text-sm">
                  - &#8377;0.1
                </span>
              </p>
            </div>
            <div className="w-full flex items-center justify-between">
              <div className="flex flex-col items-start">
                <p className="font-normal text-xs text-[#5F5F5F]">Investment</p>
                <p className="font-inter font-semibold text-xl">&#8377;5.5</p>
              </div>
              <div className="flex flex-col items-end">
                <p className="font-normal text-xs text-[#5F5F5F]">
                  Current Value
                </p>
                <p className="font-inter font-semibold text-xl">&#8377;5.5</p>
              </div>
            </div>
            <div className="w-full flex items-center justify-between gap-4 text-xs font-medium">
              <div className="w-[50%] text-center rounded-[20px] border py-2.5 px-4 bg-white text-[#2C2D32]">
                Invest
              </div>
              <div className="w-[50%] text-center rounded-[20px] border py-2.5 px-4 bg-[#2C2D32] text-white">
                Exit
              </div>
            </div>
            <div className="w-full flex justify-between items-center pt-4 border-dashed border-t-[0.7px] text-[#2C2D32]">
              <span className="font-normal text-xs">Exited Returns</span>
              <span className="font-inter text-xs font-medium">- &#8377;3</span>
            </div>
            <div className="w-full flex justify-between items-center border-dashed border-t-[0.7px] pt-4">
              <div className="flex items-center gap-2 border rounded-full">
                <span className="py-1.5 px-3.5 border-r font-normal font-inter text-[10px] text-[#2C2D32]">
                  5 unmatched
                </span>

                <span className="p-1.5 pr-3 pl-0">
                  <img src={CircleCrossIcon} alt="" />
                </span>
              </div>
            </div>
          </div>
        </div>
        <div className="w-full overflow-x-auto scrollbar-hide ">
          <Tabs
            className="w-full py-4 font-[500] text-xs"
            value={activeMarketHoldingPortfolioTab}
            onValueChange={handleTabChange}
          >
            <div className="bg-[#F5F5F5]">
              <TabsList className="flex flex-nowrap w-max rounded-full space-x-2 bg-transparent text-[#2C2D32] p-0 h-6 pl-4 pr-4">
                {['all', 'matched', 'exiting', 'exited', 'cancelled'].map(
                  (tab) => (
                    <TabsTrigger
                      key={tab}
                      value={tab}
                      ref={tabRefs[tab]}
                      className={`flex items-center flex-shrink-0 px-5 py-1.5 space-x-2 rounded-full border-[0.5px] border-[#CBCBCB] bg-white data-[state=active]:border-[0.7px] data-[state=active]:border-[#5F5F5F] text-sm text-[#5F5F5F] font-normal h-auto`}
                    >
                      <span className="whitespace-nowrap capitalize">
                        {tab}
                      </span>
                      {activeMarketHoldingPortfolioTab === tab && (
                        <img src={X} alt="Close" />
                      )}
                    </TabsTrigger>
                  )
                )}
              </TabsList>
            </div>
          </Tabs>
        </div>
      </div>
    </div>
  )
}

export default ActiveMarketHoldings
